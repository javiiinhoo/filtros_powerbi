...
ligas_disponibles = df['LIGA'].dropna().unique()  # Asegura que no haya valores nulos
...
if df_filtrado.empty:
    ...
    metricas_generales = ['VALORACION AJUSTADA DEFENSIVA', 'VALORACION DEFENSIVA','VALORACION AJUSTADA OFENSIVA', 'VALORACION OFENSIVA', 'NOTA AJUSTADA', 'NOTA']
    ...
    for metrica in metricas_fisicas:
        if df_filtrado[metrica].notna().any():
            filtros_fisicos[metrica] = st.slider(f'Selecciona el rango para {metrica}', 0, 10, (0, 10))

    # Aplicar filtros físicos
    for metrica, (min_val, max_val) in filtros_fisicos.items():
        df_filtrado = df_filtrado[(df_filtrado[metrica] >= min_val) & (df_filtrado[metrica] <= max_val)]
    ...
    for metrica in metricas_defensivas:
        if df_filtrado[metrica].notna().any():
            filtros_defensivos[metrica] = st.slider(f'Selecciona el rango para {metrica}', 0, 10, (0, 10))
    ...
    for metrica in metricas_ofensivas:
        if df_filtrado[metrica].notna().any():
            filtros_ofensivos[metrica] = st.slider(f'Selecciona el rango para {metrica}', 0, 10, (0, 10))
    ...
    for metrica in metricas_generales:
        if df_filtrado[metrica].notna().any():
            filtros_generales[metrica] = st.slider(f'Selecciona el rango para {metrica}', 0, 10, (0, 10))
    ...
    st.dataframe(df_filtrado[['ID', 'NOMBRE JUGADOR', 'EQUIPO', 'LIGA','VALORACION DEFENSIVA', 'VALORACION OFENSIVA', 'NOTA']])

    # Mostrar los comentarios de cada jugador
    jugador_seleccionado = st.selectbox('Selecciona un jugador para ver los comentarios', df_filtrado['NOMBRE JUGADOR'].unique())

    # Filtrar los comentarios para el jugador seleccionado
    comentarios = df_filtrado[df_filtrado['NOMBRE JUGADOR'] == jugador_seleccionado]['COMENTARIOS_UNIFICADOS'].values
